...
**Preview flow**: `onSearchItemSelected` fetches the detail document for the selected row and renders it into `self.webView` (a `wx.html2.WebView`, absent on Flatpak/webkit-less Linux — the pane degrades to a `wx.StaticText`). The Std branch reads `result` from `GET /api/components/{uuid}` and shows two `<figure>`s side by side, **Symbol** (`thumbUrl(result, uuid)`) and **Footprint** (`thumbUrl(packageDetail, packageDetail.uuid)`), above a table of `dataStr.head.c_para`. `thumbUrl` falls back to `https://image.easyeda.com/components/<uuid>.png` when the JSON `thumb` field is null — which it always is for `packageDetail`. Each `<img>` carries an `onerror` that hides its figure, so a document EasyEDA has not rendered drops out silently instead of needing a probe request. Detail-fetch failures are logged with `warning(...)` so the pane can never go blank without a trace.
